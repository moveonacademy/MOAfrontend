...
const Page = () => {
  ...
  var [dateDisponibilidad,] = useState(null);
  ...
  async function handleTeacher() {
    ...
    try {
      ...
      if (res) {
        ...
        if (dateDisponibilidad) {
          let dates2 = [];
          for (let i = 0; i < rowsDate.length; rowsDate++) {
            dates2 = [...dates2, rowsDate[i].date.toString()];
          }

          res.set("teacherAvaliableTimes", dates2);
        } else {
          setError("Faltan las dias de disponibilidad del profesor");
          setLoading(false);
...
=======
>>>>>>> 6a346e5540a0b81e52397ff2cd4233afecdc67c3
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DateTimePicker
